<template>
  ...
  <!-- Confirm Delete Formation Dialog -->
  <ConfirmDialog
    :show="showDeleteConfirm"
    title="Delete Formation"
    :message="`Are you sure you want to delete formation '${formationToDelete?.name}'? This action cannot be undone.`"
    confirm-text="Delete Formation"
    @confirm="handleDeleteFormation"
...
const deleteSavedFormation = (formationId: string) => {
  const formation = savedFormations.value.find(f => f.id === formationId)
  if (formation) {
...
const handleDeleteFormation = () => {
  ...
  const index = savedFormations.value.findIndex(f => f.id === formationToDelete.value!.id)
  if (index !== -1) {
      savedFormations.value.splice(index, 1)
      localStorage.setItem(`formations_${dialogType.value}`, JSON.stringify(savedFormations.value))
    if (selectedSavedFormation.value?.id === formationToDelete.value.id) {
